...
export class AuthController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad Request: Invalid input data (e.g. missing email/password).',
  ...
  async login(@Body() loginUserDto: LoginUserDto): Promise<LoginResponseDto> {
    const { access_token, userName } = await this.authService.login(loginUserDto.cpf, loginUserDto.nameWar);
